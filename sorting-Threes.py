...
def grouping():
    extras = numberNames%3 ## need to make sure the groups are 2 in size
    threeGroups = numberNames/3
    print threeGroups
    if extras == 1:  ## if there is 1 extra, get a 2nd from one group of 3
        threeGroups = threeGroups - 1
        numGroups = (threeGroups, 2)
    elif extras == 2: ## two extras is ok
        threeGroups = threeGroups + 1
        numGroups = (threeGroups,0)
    else:
        numGroups = (threeGroups,0)
    print numGroups
    print extras
...
def sort(bounces,person,NN,groups):
    ...
    for two in range(groups[1]):
        groupNumbers.append(2)

    ## put kids into each group
    for j in range(groups[0] + groups[1]):
        h = 70
        w.create_text(j*93+60,20,text="Group "+str(j+1), fill="red",font=fontz)
        for k in range(groupNumbers[gn]):
            if i < NN:
                ...
                movename(sortName[i],(j*93+60),h,rX[i],rY[i])
                h = h + 50
                i = i + 1
        gn = gn + 1
        w.delete(i+bounces*NN)
